...
void Task_scheduler_my(string algo_cmd, int batch_window, vector<Task> Request_queue) {
    // thread
    pthread_t p_thread[3]; // device number
    pthread_attr_t attr;
    int status;
    int thr_id;
    ...
    // pthread detached (for resource manangmenet
	...
	long int sch_start_time = tp.tv_sec; 
	long int sch_end_time;
	...
	if(BIG_queue.size() != 0 && BIG_RUN == 0) {
		int s_idx = -1; // selected idx
       		...
       		gettimeofday(&tp, NULL);  // Added
		BEFORE = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
		...
		task->arrival_time = BIG_queue[s_idx].arrival_time;
		task->dev = BIG_queue[s_idx].dev;
		task->task_idx = BIG_queue[s_idx].task_idx;
		task->after_batch_decision_time = BIG_queue[s_idx].after_batch_decision_time;
		task->deadline = BIG_queue[s_idx].deadline;

		task->is_vio = BIG_queue[s_idx].is_vio;
		task->est_latency = BIG_queue[s_idx].est_latency;
		task->est_runtime = BIG_queue[s_idx].est_runtime;
		task->real_arrival_time = BIG_queue[s_idx].real_arrival_time;

/*
       		gettimeofday(&tp, NULL);  // Added
		AFTER = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
    		Write_file_BIG << "COPY OVERHEAD: " << AFTER - BEFORE << endl;
...
/*
       			gettimeofday(&tp, NULL);  // Added
    			Write_file_BIG << "RUN BEFORE: " << tp.tv_sec* 1000 + tp.tv_usec / 1000 << endl;
*/

    			thr_id = pthread_create(&p_thread[0], &attr, pthread_execute_layer0, (void*)task);
			BIG_queue.erase(BIG_queue.begin() + s_idx);
		}
       		gettimeofday(&tp, NULL);  // Added
		AFTER = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
		sum_BIG_overhead += (AFTER - BEFORE);
    		//Write_file_BIG << "THREAD CREATION: " << AFTER - BEFORE << endl;
	}
	if(GPU_queue.size() != 0 && GPU_RUN == 0) {
		...
		task->arrival_time = GPU_queue[s_idx].arrival_time;
		task->dev = GPU_queue[s_idx].dev;
		task->task_idx = GPU_queue[s_idx].task_idx;
		task->after_batch_decision_time = GPU_queue[s_idx].after_batch_decision_time;
		task->deadline = GPU_queue[s_idx].deadline;

		task->is_vio = GPU_queue[s_idx].is_vio;
		task->est = GPU_queue[s_idx].est;
		task->est_latency = GPU_queue[s_idx].est_latency;
		task->est_runtime = GPU_queue[s_idx].est_runtime;
		task->real_arrival_time = GPU_queue[s_idx].real_arrival_time;
		task->gpu_freq = GPU_queue[s_idx].gpu_freq;
       		//gettimeofday(&tp, NULL);  // Added
		//CUR_TIME = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
		//cout << "BEFORE pthread_exeucte_layer0()\t|\t" << "WAIT: " << task->id << task->task_idx << ":" << CUR_TIME - task->after_batch_decision_time << endl;

		//cout << "VALUE2: " << task->after_batch_decision_time << endl;

/*
       		gettimeofday(&tp, NULL);  // Added
		AFTER = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
    		//Write_file_GPU << "COPY OVERHEAD: " << AFTER - BEFORE << endl;
...
/*
       			gettimeofday(&tp, NULL);  // Added
    			Write_file_GPU << "RUN BEFORE: " << tp.tv_sec* 1000 + tp.tv_usec / 1000 << endl;
*/
    			thr_id = pthread_create(&p_thread[1], &attr, pthread_execute_layer0, (void*)task);
			GPU_queue.erase(GPU_queue.begin() + s_idx);

		}
       		gettimeofday(&tp, NULL);  // Added
		AFTER = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
		sum_GPU_overhead += (AFTER - BEFORE);
    		//Write_file_GPU << "THREAD CREATION: " << AFTER - BEFORE << endl;
	...
	if(DSP_queue.size() != 0 && DSP_RUN == 0) {
		...
		task->arrival_time = DSP_queue[s_idx].arrival_time;
		task->dev = DSP_queue[s_idx].dev;
		task->task_idx = DSP_queue[s_idx].task_idx;
		task->after_batch_decision_time = DSP_queue[s_idx].after_batch_decision_time;
		task->deadline = DSP_queue[s_idx].deadline;

		task->is_vio = DSP_queue[s_idx].is_vio;
		task->est_latency = DSP_queue[s_idx].est_latency;
		task->est_runtime = DSP_queue[s_idx].est_runtime;
		task->real_arrival_time = DSP_queue[s_idx].real_arrival_time;

/*
       		gettimeofday(&tp, NULL);  // Added
		AFTER = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
    		Write_file_DSP << "COPY OVERHEAD: " << AFTER - BEFORE << endl;
...
/*
       			gettimeofday(&tp, NULL);  // Added
    			Write_file_DSP << "RUN BEFORE: " << tp.tv_sec* 1000 + tp.tv_usec / 1000 << endl;
*/
    			thr_id = pthread_create(&p_thread[2], &attr, pthread_execute_layer0, (void*)task);
			
			DSP_queue.erase(DSP_queue.begin() + s_idx);
		}
       		gettimeofday(&tp, NULL);  // Added
		AFTER = tp.tv_sec * 1000 + tp.tv_usec / 1000; 
		sum_DSP_overhead += (AFTER - BEFORE);	
    		//Write_file_DSP << "THREAD CREATION: " << AFTER - BEFORE << endl;
	}
	//usleep(1000 * 1000); // 1000ms
	//usleep(1000 * 10); // 10ms
	//usleep(1000 * 1); // 5ms
	//usleep(1000 * 1); // 1ms ??? possible?????????????????????????

	//for(int i = 0; i < DSP_queue.size(); i++){
	//	DSP_queue[i].wait_time += 1;
	//}
    } 

		
       	gettimeofday(&tp, NULL);  // Added
	sch_end_time = tp.tv_sec; 
   ...
   //cout << "Queue: " << BIG_queue.size() << " " << GPU_queue.size() << " " <<  DSP_queue.size() <<  " = " <<  BIG_queue.size() + GPU_queue.size() + DSP_queue.size()  << endl; 
   //cout << "Finished: " << Finished_BIG << " " <<  Finished_GPU << " " <<  Finished_DSP  << " = " << Finished_BIG + Finished_GPU + Finished_DSP  << " / " << total_task << endl ;
    ...
    if(BIG_check == 1)
    	pthread_join(p_thread[0], (void **)&status); // BIG
    if(GPU_check == 1)
    	pthread_join(p_thread[1], (void **)&status); // GPU
    if(DSP_check == 1)
   	pthread_join(p_thread[2], (void **)&status); // DSP
